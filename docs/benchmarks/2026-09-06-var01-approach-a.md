...
clean wipe of `build/opt` + `tmp/bench-out`, then
...
Tree: `known-issues-fixes` @ `a254810` (algorithm tip `2732aca`: corner-anchored
`sphereIntersectsCellBox` + `kLocalSearchExpansionCap = 3000` with one full-map fallback).
