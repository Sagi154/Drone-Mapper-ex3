...
| Y11 per-run naming | ⏳ unblocked | Pattern documented in `README.md` — Yoav aligns remaining per-run error-log names |
...
### Y11 — Per-run output file naming (unblocked)
Follow the pattern in `README.md`. Remaining work is mainly wiring per-run
`<plugin>_run_NNNN_error.log` (maps already use that stem).

**OQ-Y1 ✅ done in tree:** empty `output_path` skips save; exception containment in place.

Best remaining uses of time:
- Y11 error-log naming
...
| Who | What | Why now |
|---|---|---|
| **Yoav** | Y11 per-run error-log naming (follow `README.md`) | Unblocked; last Simulator naming piece |
